...
class ObjectDetectorOptions(NamedTuple):
    ...
    """Enable the use of Non-maximum Suppression (NMS) prune overlapping boxes"""

    NMSBoxes_threshold: float = 0.5
    """The score threshold of NMS algorithm (smoothing algorithm)."""
...
class ObjectDetector:
    ...
    def __init__(
        ...
        if options.enable_edgetpu:
            ...
            interpreter = tf.lite.Interpreter(
                ...
                experimental_delegates=[tf.lite.experimental.load_delegate(edgetpu_lib_name())],
    ...
    def _postprocess(self, boxes: np.ndarray, classes: np.ndarray,
		...
		# Apply Non Max Suppression to prune overlapping boxes 
        if self._options.smooth_boxes:
            if filtered_results:
                temp_results = []
                boxes = []
                scores = []
                ...
                for detection in filtered_results:
                    ...
                    scores.append(detection.categories[0].score)
                
                indices = tf.image.non_max_suppression(
                    boxes = boxes,
                    scores = scores,
                    max_output_size = self._options.max_results,
                    iou_threshold = self._options.NMSBoxes_threshold
                )
                
                for index in indices:
                    temp_results.append(filtered_results[index])
...
def main():
    for input_image in glob.glob(f"{INPUT_FILE}*.jpg"):
        ...
        options = ObjectDetectorOptions(
            ...
            smooth_boxes=True,
            NMSBoxes_threshold=NMS_THRESHOLD,
        )
